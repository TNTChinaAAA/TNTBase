...
public class Utils extends Gui {
    ...
    public void breakBlockLegit(final int posX, final int posY, final int posZ, final int delay) {
        ...
        if (this.hitDelay >= delay) {
            ...
            if (this.blockDamage >= (this.mc.playerController.isInCreativeMode() ? 0.0f : 1.0f)) {
                ...
                this.mc.playerController.onPlayerDestroyBlock(this.getBlockPos(posX, posY, posZ), this.getEnumFacing(posX, posY, posZ));
    ...
    public boolean canBlockBeSeen(final int posX, final int posY, final int posZ) {
        ...
        return result == null || result.getBlockPos() != this.getBlockPos(posX, posY, posZ) || result.sideHit != null;
    }
    ...
    public void attackTarget(final EntityLivingBase entity) {
        final float sharpLevel = EnchantmentHelper.getModifierForCreature(this.mc.thePlayer.getHeldItem(), entity.getCreatureAttribute());
        this.mc.thePlayer.swingItem();
