...
public class RenderUtils {
	...
	public static void drawCenteredString(String s, int x, int y, int colour) {
		x -= Minecraft.getMinecraft().fontRendererObj.getStringWidth(s) / 2;
		Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s, x, y, colour);
	}

	public static void drawRect(double x1, double y1, double x2, double y2, int color) {
		GuiUtil.drawRect(x1, y1, x2, y2, color);
	}
	...
	public static void drawRectWH(double x, double y, double width, double height, int color) {
		GuiUtil.drawRect(x, y, x + width, y + height, color);
	}
	...
	public static void drawRainbowRect(double x, double y, double x2, double y2) {
		...
		for (int i = 0; i <= (width); i++) {
			Color color = new Color(
					Color.HSBtoRGB((float) ((Minecraft.getMinecraft().thePlayer.ticksExisted / width) / 2
							+ (Math.sin(i / width * 0.6))) % 1.0f, 0.5f, 1.0f));
	...
	public static class R2DUtils {
		...
		public static ResourceLocation loadHead(final EntityPlayer player) {
			...
			final ThreadDownloadImageData textureHead = new ThreadDownloadImageData(null,
					String.format("https://minotar.net/helm/%s/64.png", player.getName()), null, null);
		...
		public static void drawRainbowRect(double x, double y, double x2, double y2) {
			double width = (x2 - x);
			for (int i = 0; i <= (width); i++) {
				Color color = new Color(Color.HSBtoRGB((float) ((Minecraft.getMinecraft().thePlayer.ticksExisted / width) / 2 + (Math.sin(i / width * 0.6))) % 1.0f, 0.5f, 1.0f));
				drawRect(x + i, y, x + i + 1, y2, color.getRGB());
			}
		}
		...
		public static double[] getScreenCoords(final double x, final double y, final double z) {
			...
			final boolean result = GLU.gluProject((float) x, (float) y, (float) z, modelView, projection, viewport,
					screenCoords);
			if (result) {
				final double[] coords = { screenCoords.get(0), Display.getHeight() - screenCoords.get(1),
						screenCoords.get(2) };
				return coords;
			}
			return null;
		...
		/**
		 * Draws a bordered rectangle at the coordinates specified with the hexadecimal
		 * color.
		 */
		...
		public static void drawFilledCircle(int x, int y, double radius, Color c) {
			...
			/*for (double i = 0; i < 360; i++) {
				...
				double[] outer = new double[] { Math.cos(cs) * radius, -Math.sin(cs) * radius, Math.cos(ps) * radius,
						-Math.sin(ps) * radius };
		...
		/*public static void RenderLivingEntityBox(Entity entity, float partialTicks, boolean otherMode) {
			...
			if (otherMode) {
				RenderGlobal.drawOutlinedBoundingBox(
						new AxisAlignedBB(entity.boundingBox.minX - (isPlayer ? 0.12 : 0) - entity.posX + (posX),
								entity.boundingBox.minY - (entity.isSneaking() ? entity.posY + 0.2 : entity.posY)
										+ (posY),
								entity.boundingBox.minZ - (isPlayer ? 0.12 : 0) - entity.posZ + (posZ),
								entity.boundingBox.maxX + (isPlayer ? 0.12 : 0) - entity.posX + (posX),
								entity.boundingBox.maxY + (isPlayer ? 0.2 : 0)
										- (entity.isSneaking() ? entity.posY + 0.2 : entity.posY) + (posY),
								entity.boundingBox.maxZ + (isPlayer ? 0.12 : 0) - entity.posZ + (posZ)),
						-1);
				drawLines(new AxisAlignedBB(entity.boundingBox.minX - (isPlayer ? 0.12 : 0) - entity.posX + (posX),
		...
		public static void drawRainbowRect(double x, double y, double x2, double y2) {
			...
			for (int i = 0; i <= (width); i++) {
				Color color = new Color(Color.HSBtoRGB(
						(float) ((Minecraft.getMinecraft().thePlayer.ticksExisted / width) / 2 + (Math.sin(i / width * 0.6))) % 1.0f,
						0.5f, 1.0f));
				drawRect(x + i, y, x + i + 1, y2, color.getRGB());
			}
		}
	...
	public static final class Stencil {
		...
		public void stopLayer() {
			...
			if (layers == 1) {
				...
				Stencil.StencilFunc lastStencilFunc = (Stencil.StencilFunc) stencilFuncs
						.remove(Integer.valueOf(layers));
		...
		public void setBuffer() {
			setStencilFunc(new Stencil.StencilFunc(this, !renderMask ? 512 : 519, layers, getMaximumLayers(), 7681,
					7680, 7680));
		}

		public void setBuffer(boolean set) {
			setStencilFunc(new Stencil.StencilFunc(this, !renderMask ? 512 : 519, set ? layers : layers - 1,
		...
		public static class StencilFunc {
			...
			public int func_func;
			public int func_ref;
			public int func_mask;
			public int op_fail;
			public int op_zfail;
			public int op_zpass;

			public StencilFunc(Stencil paramStencil, int func_func, int func_ref, int func_mask, int op_fail,
					int op_zfail, int op_zpass) {
	...
	public static class Camera {
		...
		public Camera(Entity entity) {
			...
			this.posX = entity.lastTickPosX
					+ (entity.posX - entity.lastTickPosX) * (double) this.timer.renderPartialTicks;
			this.posY = entity.lastTickPosY
					+ (entity.posY - entity.lastTickPosY) * (double) this.timer.renderPartialTicks;
			this.posZ = entity.lastTickPosZ
					+ (entity.posZ - entity.lastTickPosZ) * (double) this.timer.renderPartialTicks;
		...
		public Camera(Entity entity, double offsetX, double offsetY, double offsetZ, double offsetRotationYaw,
				double offsetRotationPitch) {
			this.posX = entity.lastTickPosX
					+ (entity.posX - entity.lastTickPosX) * (double) this.timer.renderPartialTicks;
			this.posY = entity.lastTickPosY
					+ (entity.posY - entity.lastTickPosY) * (double) this.timer.renderPartialTicks;
		...
		public static float[] getRotation(double posX1, double posY1, double posZ1, double posX2, double posY2,
				double posZ2) {
