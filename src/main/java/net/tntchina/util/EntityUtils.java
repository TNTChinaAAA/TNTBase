...
public class EntityUtils {
	...
	public double getDistanceToFall(Entity e){
		...
		for(double i = e.posY; i > 0; i -= 0.1){
			if(i < 0)
				break;
			Helper.blockUtils();
			Block block = BlockUtils.getBlock(new BlockPos(e.posX, i, e.posZ));
	 ...
	 public Entity findClosestToCross(double range) {
		    ...
		    for (Object o : Minecraft.getMinecraft().theWorld.loadedEntityList) {
		      ...
		      if (!(ent instanceof EntityPlayer)) continue;
			 	 ...
			 	 Helper.combatUtils();
				double difference = Math.abs(CombatUtils.angleDifference(newYaw, Minecraft.getMinecraft().thePlayer.rotationYaw));
    ...
    public static boolean isEntityWithinView(Entity entityLiving, float scope) {
  	  ...
  	  Helper.combatUtils();
	final double difference = CombatUtils.angleDifference(newYaw, Minecraft.getMinecraft().thePlayer.rotationYaw);
    ...
    public boolean isWithingFOV(Entity en, float angle) {
        ...
        Helper.combatUtils();
		Helper.entityUtils();
		double angleDifference = CombatUtils.angleDifference(Helper.player().rotationYaw , EntityUtils.getRotationToEntity(en)[0]);
